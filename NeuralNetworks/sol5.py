...
Original file is located at
    https://colab.research.google.com/drive/1_2_e3-Pjgs1j12mhwVAMtlvS_3sXsu8v
...
cs_id = "yairabraham5" #@param {type:"string"}
...
def load_dataset(filenames, batch_size, corruption_func, crop_size):
    ...
    while True:
      ...
      corrupted_images = []
      image = None
      # all_indexes = np.random.choice(range(len(filenames)), batch_size)
      # rand_files = filenames[all_indexes]
      for i in (range(batch_size)):
        rand_file = filenames[np.random.randint(0, len(filenames))]
        if rand_file in all_files:
          image = all_files[rand_file]
        else:
          image = read_image(rand_file, 1)
          all_files[rand_file] = image
        image = np.array(image)
        x_crop_size = np.int(crop_size[0])
        y_crop_size = np.int(crop_size[1])
        # change the size of the image by cropping
        x_cord_fc = np.random.randint(3* crop_size[0], image.shape[0] - 3*crop_size[0])
        y_cord_fc = np.random.randint(3*crop_size[1], image.shape[1] - 3*crop_size[1])
        cropped_image = image[x_cord_fc: x_cord_fc +  3* x_crop_size, y_cord_fc: y_cord_fc +  3*y_crop_size]
        dirty_image = corruption_func(cropped_image)
        x_cord_sc = np.random.randint(crop_size[0], cropped_image.shape[0] - crop_size[0] + 1)
        y_cord_sc = np.random.randint(crop_size[1], cropped_image.shape[1] - crop_size[1] + 1)
        final_clean_image = cropped_image[x_cord_sc: x_cord_sc  + x_crop_size, y_cord_sc: y_cord_sc + y_crop_size]
        final_dirty_image = dirty_image[x_cord_sc: x_cord_sc  + x_crop_size, y_cord_sc: y_cord_sc + y_crop_size]
        final_clean_image = final_clean_image.reshape(final_clean_image.shape + (1,)) - 0.5
        clean_images.append(final_clean_image)
        final_dirty_image = final_dirty_image.reshape(final_dirty_image.shape + (1,)) - 0.5
        corrupted_images.append(final_dirty_image)
      yield (np.array(corrupted_images), np.array(clean_images))
...
def train_model(model, images, corruption_func, batch_size, steps_per_epoch, num_epochs, num_valid_samples):
    ...
    """
    training_set, validation_set = train_test_split(images, train_size=0.8, test_size=0.2)
    data_training_set = load_dataset(training_set, batch_size, corruption_func, model.input_shape[1:3])
    data_valid_set = load_dataset(validation_set, batch_size, corruption_func, model.input_shape[1:3])
    model.compile(loss='mean_squared_error', optimizer=Adam(beta_2=0.9))
    model.fit_generator(generator=data_training_set, steps_per_epoch=steps_per_epoch, epochs=num_epochs ,validation_data=data_valid_set, validation_steps=num_valid_samples)
...
def restore_image(corrupted_image, base_model):
    ...
    corrupted_imaged = corrupted_image.reshape((1,) + corrupted_image.shape + (1,))
    y = new_model.predict(corrupted_imaged - 0.5)
    y = y.reshape(corrupted_image.shape)
    y = y + 0.5
...
def add_gaussian_noise(image, min_sigma, max_sigma):
    ...
    values = np.random.normal(0, sigma, image.shape)
    noisy_image = image + values
    # rounding to the nearest i/255 fraction
    noisy_image = (np.round(noisy_image * 255))/255
...
def learn_denoising_model(denoise_num_res_blocks, quick_mode=False):
    ...
    """
    all_images = images_for_denoising()
    # our corruption function
    corrup_func = lambda image: add_gaussian_noise(image, 0, 0.2)
    # model 
    model = build_nn_model(24, 24, 48, denoise_num_res_blocks)
    if not quick_mode:
      train_model(model, all_images, corrup_func, 100, 100, 10, 1000//100)
    else:
      train_model(model, all_images, corrup_func, 10, 3, 2, 30)
    return model
...
def random_motion_blur(image, list_of_kernel_sizes):
    ...
    random_kernel_size = list_of_kernel_sizes[np.random.randint
                                              (0, len(list_of_kernel_sizes))]
    blurred_image = add_motion_blur(image, random_kernel_size, angle)
    # rounding to the nearest i/255 fraction
...
deblur_num_res_blocks = 1 #@param {type:"slider", min:1, max:15, step:1}
...
def learn_deblurring_model(deblur_num_res_blocks, quick_mode=False):
    ...
    :return: the trained model
    """
    all_images = images_for_deblurring()
    # our corruption function
    corrup_func = lambda image: random_motion_blur(image, [7])
    # model 
...
def super_resolution_corruption(image):
    ...
    """
    # get the zoom factor
    zoom_float = np.random.randint(2,5)
    zoom_image = zoom(image, 1/zoom_float)
    zoomed_image = zoom(zoom_image, zoom_float)
    final_image = map_coordinates(zoomed_image, np.indices(image.shape))
    np.clip(final_image, 0, 1, out=final_image)
    return final_image

#@markdown ### 7.3.2 Training a Super Resolution Model


super_resolution_num_res_blocks = 4 #@param {type:"slider", min:1, max:30, step:1}
batch_size = 100 #@param {type:"slider", min:1, max:128, step:1}
steps_per_epoch = 100 #@param {type:"slider", min:10, max:5000, step:10}
...
def learn_super_resolution_model(super_resolution_num_res_blocks, quick_mode=False):
    ...
    :return: the trained model
    """
    all_images = images_for_super_resolution()
    # our corruption function
    corrup_func = lambda image: super_resolution_corruption(image)
    # model 
    model = build_nn_model(patch_size, patch_size, num_channels, super_resolution_num_res_blocks)
    if not quick_mode:
      train_model(model, all_images, corrup_func, batch_size, steps_per_epoch, num_epochs, 1000//batch_size)
    else:
        train_model(model, all_images, corrup_func, 10, 3, 2, 30)
    return model
...
# Answer1 = "This implementation will work for the follow reasons. Firstly, the corruption function works because we want to \"ruin\" the resolution of the image. We do that by zooming out, when we zoom out we \"erase\" some of the pixels. After that we zoom in but we have to guess some of the pixels making the zoomed in image corrupted. It will work because the learning model is written like the deblurring learning model. " #@param {type:"string"}
...
# Answer2 = "I tried by using the given parameters of the deblurring model, this worked out okay. I tweaked some of the parameters to get better results. I changed the patch size and the num_channels and this lowered the error rate of the training proccess but the training itself took longer. The final parameters are super_resolution_num_res_blocks = 4, batch_size = 100, steps_per_epoch = 100, num_epochs = 10, patch_size = 32, num_channels = 64." #@param {type:"string"}
...
Answer3 = "The effect of increasing the residual blocks makes the loss of the test set smaller in the deblurring model but not always in the loss in the denoising model. We can see in the deblur plot that as we increase the number of residual blocks the mse is smaller and also image wise when we look at a deblurred image as has been restored we can see a vast difference between a deblurring model that had lower residual blocks and one that had higher residual blocks. On the other hand we can see that there is some kind of overfitting in the denoising model as we get higher residual blocks the mse in the graph gets higher. In images we can see the image that was restored from the model with 5 residual blocks looked worse than a restored image that had 4 residual blocks." #@param {type:"string"}
